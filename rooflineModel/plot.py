...
class Roofline:
    ...
    def __init__(self, alpha, beta, xaxis, yaxis, title=None, xlim=None, ylim=None, axis=None):
        ...
        self.constBoxMultiplier = 1 #00
        if xlim is not None:
            self.xstart, self.xend = xlim
        else:
            self.xstart = 0.001
            self.xend = 1 #0.001
        if ylim is not None:
            self.ystart, self.yend = ylim
        else:
            self.ystart = 0.01
            self.yend = 1 #0.01
        self.xlim = xlim
        ...
        self.annotateOffsetX = -3
        self.annotateOffsetY = 12
    ...
    def addAmortized(self, name, alphaName, betaName, start=None, end=None):
        """
        ...
        self.amortized[name] = (alphaName, betaName, start, end)

    def plotAmortized(self, name, alphaName, betaName, start, end):
        """
        ...
        points = 1000
        a = math.ceil(math.log10(start))
        b = math.floor(math.log10(end))
    ...
    def addAlongAmortized(self, alphaName, betaName, dataName, x, pointLabels=None):
        alpha = [a for a in self.alphas if a[0] == alphaName][0]
        beta = [b for b in self.betas if b[0] == betaName][0]
        y = [(alpha[2]*beta[1]*X)/(alpha[2] + beta[1]*X) for X in x]
        if pointLabels is None:
            self.addData({dataName:(x,y)})
        else:
            self.addLabelData({dataName:{l:(X,Y) for l,X,Y in zip(pointLabels, x, y)}})
    ...
    def annotateLabel(self, x, y, label):
        ...
        self.axis.annotate(
            ...
            xy=(x, y), 
            # xytext=(-10,10), 
            xytext=(self.annotateOffsetX,self.annotateOffsetY), 
            ha='left', va='top',
    ...
    def plot(self, saveFile=None, xlim=None, ylim=None):
        ...
        for series in self.amortized:
            self.plotAmortized(series, self.amortized[series][0], self.amortized[series][1], self.amortized[series][2], self.amortized[series][3])
